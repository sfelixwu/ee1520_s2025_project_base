      ...
      if (strcmp(buff, "\"id\"") == 0)
	{
	  ...
	  for (j = 0; j < strlen(sd_ptr); j++)
		  ...
		  (j < 128))
		{
	      ...
	      if (j >= 128)
		{
		  return EE1520_ERROR_VSID_POST_ID;
		}
...
(Json::Value * arg_json_ptr)
{
  ...
  JSON2Object_precheck(arg_json_ptr, lv_exception_ptr,
		       EE1520_ERROR_JSON2OBJECT_POST);

  // the following line is ONLY for Post.cpp. We keep a copy of the incoming JSON.
  // for debugging
  this->J2O_input = (*arg_json_ptr);
  ...
  if ((((*arg_json_ptr)["id"]).isNull() == true) ||
      ...
      ((((*arg_json_ptr)["id"]).asString()).size() > 255))
    {
      ...
      ei_ptr->where_code = EE1520_ERROR_JSON2OBJECT_POST;
      ...
      if (checkPostID(((*arg_json_ptr)["id"]).asString()) != 0)
	{
	  ...
	  ei_ptr->which_string = "id";
	  ei_ptr->how_code = EE1520_ERROR_NORMAL;
	  ei_ptr->what_code = EE1520_ERROR_POST_ID_CHECK_FAILED;
	  ei_ptr->array_index = 0;
  ...
  if ((((*arg_json_ptr)["comments"]).isNull() == true) ||
      ...
      else
	{
      ...
      // for Comments, it is okay without any comment
      if (((*arg_json_ptr)["comments"]).isNull() == false)
	{
	  (lv_exception_ptr->info_vector).push_back(ei_ptr);
	}
      else
	{
	  // if NULL, we should free the ei_ptr
	  delete ei_ptr;
	}
    }
  else
      ...
      else
	{
	  ...
	  for (ai = 0; ai < ((*arg_json_ptr)["comments"]["data"]).size(); ai++)
		   ...
		   my_it_co < (*(this->comments)).end(); my_it_co++)
		{
	      ...
	      // printf("Post Here 10 6\n");
	      if (flag_co == 0)
		{
		  // printf("Post Here 10 7\n");
  ...
  if ((((*arg_json_ptr)["reactions"]).isNull() == true) ||
      ...
      else
	{
      ...
      // No reactions should be fine so no exception raised
      // (lv_exception_ptr->info_vector).push_back(ei_ptr);
      delete ei_ptr;
    }
  else
  ...
  if (((*arg_json_ptr)["is published"].isNull() == true) ||
    ...
    {
      this->is_published = ((*arg_json_ptr)["is published"]).asBool();
    }
