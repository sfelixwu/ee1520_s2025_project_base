...
main(int argc, char *argv[])
{
  ...
  try
      ...
      for (i = 0; i < person_vector_ptr->size(); i++)
	{
	  ...
	  std::cout << (h_current->dump2JSON())->toStyledString() << std::endl;
      ...
      if ((beer.product)->keyword == "alcohol")
	{
      ...
      std::cout << *(c1.dump2JSON()) << std::endl;
  
      p1.message(*(c1.dump2JSON()));
      p1.message(*(c2.dump2JSON()));
    }
  catch(ee1520_Exception& e)
    {
      std::cout << "grocery JSON2Object exception\n";
